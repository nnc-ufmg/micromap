...
class plot_data_class(QObject):
    ...
    def __init__(self, plot_viewer, options, buffer):                                                                       # Initializes when the thread is started
        ...
        self.channel_sequence = self.channel_count[28:]                                                                     # Puts the last two channels at the beginning of the list to be plotted. This is because RHD gets the sample after 2 convert commands, so the result...
        self.channel_sequence += self.channel_count[:28]                                                                    # ... of the first sample arrives only on the third conversion, the result of the second sample arrives only on the fourth conversion, and so on 
        ...
        for i in self.channel_count:                                                                                        # Creates a row in the matrix for each active channel 
            ...
            self.lines.append(self.plot_viewer.canvas.axes.plot(self.time_in_seconds, self.data_matrix[i], linewidth=0.6))  # Plots lines in interface
    ...
    def plot(self):
        ...
        micro_volts_data = 100*0.195*numpy.array(integer_data)*1e-6                                                         # Transforms data to Volts (0.195*1e-6 -> datasheet) and adds a scale factor to plot (100 -> arbitrary)

        for i in self.channel_count:                                                                                        # For all channels recorded
            self.data_matrix[i].extend(self.channels[i] + micro_volts_data[i+2::self.number_channels])                      # Gets the data in Volts "channel" by "channel", puts in matrix to be plotted and for each channel add the channel number for better visualization of the data
            self.lines[self.channel_sequence[i]][0].set_ydata(self.data_matrix[self.channel_sequence[i]])                   # Plots the respective data with set_ydata (this function only changes the data without changes the axes configurations)
            self.plot_viewer.canvas.draw_idle()                                                                             # Draws on the interface
    ...
    def plot_clean(self):
        ...
        '''
        self.plot_viewer.canvas.axes.cla()                                                                                  # Clears the axes
        
        self.plot_viewer.canvas.axes.set_facecolor("#252525")                                                               # Changes the plot face color
        self.plot_viewer.canvas.axes.tick_params(colors = "#969696", grid_color = "#969696")                                # Changes the ticks design
        self.plot_viewer.canvas.axes.set_xlabel("Time [seconds]", family = "DejaVu Sans", 
                                                weight = "bold", color = "#FFFFFF", size = 6)                               # Changes the labels design 
        self.plot_viewer.canvas.axes.set_ylabel("Channels", family = "DejaVu Sans", 
                                                weight = "bold", color = "#FFFFFF", size = 6)                               # Changes the labels design 

        for spines in self.plot_viewer.canvas.axes.spines.values():                                                         # Goes through all spines
            spines.set_edgecolor('#969696')                                                                                 # Changes the spines face color
        for tick in self.plot_viewer.canvas.axes.get_xticklabels():                                                         # Goes through all tick labels in axe X
            tick.set_fontname("DejaVu Sans")                                                                                # Changes the tick labels font
            tick.set_fontweight("bold")                                                                                     # Changes the tick labels font style
            tick.set_fontsize(6)                                                                                            # Changes the tick labels font size
        for tick in self.plot_viewer.canvas.axes.get_yticklabels():                                                         # Goes through all tick labels in axe X
            tick.set_fontname("DejaVu Sans")                                                                                # Changes the tick labels font
            tick.set_fontweight("bold")                                                                                     # Changes the tick labels font style
            tick.set_fontsize(6)                                                                                            # Changes the tick labels font size
        
        self.plot_viewer.canvas.axes.yaxis.set_major_locator(MultipleLocator(1))                                            # Sets the grid on Y axe
        self.plot_viewer.canvas.axes.grid(visible=True, axis='y', which='major', alpha=0.2, ls='-', lw=0.6)                 # Sets the type of the Y axe grid
        
        self.plot_viewer.canvas.axes.set_ylim((0.1, 32.9))                                                                  # Sets limits on Y axe
        self.plot_viewer.canvas.axes.set_xlim((0, self.atualization_time))                                                  # Sets limits on X axe
...
class receive_data_class(QObject):
    ...
    def run_view(self):
        ...
        self.plot_data_worker.plot_clean()                                                                      # Clears the plot viewer
    ...
    def run_recording(self):
        ...
        self.file = open(self.options.save_directory + "_0", "wb")                                        # Opens/Creates the file to write the binary data
        ...
        while self._is_running == True:                                                                         # While _isRunning is True
            if (self.usb.port.inWaiting() >= self.bytes_to_read) and (self._is_stopped == False):               # If has any data in UART buffer waiting to be read and the record is not paused
                ...
                self.file = open(self.options.save_directory + "_" + str(self.directory_count), "wb")           # Opens/Creates the new file to write the binary data
                self._change_directory = False
...
#%% CONNECTION AND INITIALIZATION FUNCTIONS
    ...
    def __init__(self):
        ...
        self.interface = uic.loadUi(os.path.dirname(__file__) + "\\NNC_InBrain_GUI.ui", self)                                                 # Loads the interface design archive (made in Qt Designer)
        ...
        # Plot control interactions
        self.zoom_in_01_button.clicked.connect(lambda: self.zoom_in_channel(1))
        self.zoom_in_02_button.clicked.connect(lambda: self.zoom_in_channel(2))
        self.zoom_in_03_button.clicked.connect(lambda: self.zoom_in_channel(3))
        self.zoom_in_04_button.clicked.connect(lambda: self.zoom_in_channel(4))
        self.zoom_in_05_button.clicked.connect(lambda: self.zoom_in_channel(5))
        self.zoom_in_06_button.clicked.connect(lambda: self.zoom_in_channel(6))
        self.zoom_in_07_button.clicked.connect(lambda: self.zoom_in_channel(7))
        self.zoom_in_08_button.clicked.connect(lambda: self.zoom_in_channel(8))
        self.zoom_in_09_button.clicked.connect(lambda: self.zoom_in_channel(9))
        self.zoom_in_10_button.clicked.connect(lambda: self.zoom_in_channel(10))
        self.zoom_in_11_button.clicked.connect(lambda: self.zoom_in_channel(11))
        self.zoom_in_12_button.clicked.connect(lambda: self.zoom_in_channel(12))
        self.zoom_in_13_button.clicked.connect(lambda: self.zoom_in_channel(13))
        self.zoom_in_14_button.clicked.connect(lambda: self.zoom_in_channel(14))
        self.zoom_in_15_button.clicked.connect(lambda: self.zoom_in_channel(15))
        self.zoom_in_16_button.clicked.connect(lambda: self.zoom_in_channel(16))
        self.zoom_in_17_button.clicked.connect(lambda: self.zoom_in_channel(17))
        self.zoom_in_18_button.clicked.connect(lambda: self.zoom_in_channel(18))
        self.zoom_in_19_button.clicked.connect(lambda: self.zoom_in_channel(19))
        self.zoom_in_20_button.clicked.connect(lambda: self.zoom_in_channel(20))
        self.zoom_in_21_button.clicked.connect(lambda: self.zoom_in_channel(21))
        self.zoom_in_22_button.clicked.connect(lambda: self.zoom_in_channel(22))
        self.zoom_in_23_button.clicked.connect(lambda: self.zoom_in_channel(23))
        self.zoom_in_24_button.clicked.connect(lambda: self.zoom_in_channel(24))
        self.zoom_in_25_button.clicked.connect(lambda: self.zoom_in_channel(25))
        self.zoom_in_26_button.clicked.connect(lambda: self.zoom_in_channel(26))
        self.zoom_in_27_button.clicked.connect(lambda: self.zoom_in_channel(27))
        self.zoom_in_28_button.clicked.connect(lambda: self.zoom_in_channel(28))
        self.zoom_in_29_button.clicked.connect(lambda: self.zoom_in_channel(29))
        self.zoom_in_30_button.clicked.connect(lambda: self.zoom_in_channel(30))
        self.zoom_in_31_button.clicked.connect(lambda: self.zoom_in_channel(31))
        self.zoom_in_32_button.clicked.connect(lambda: self.zoom_in_channel(32))
        self.zoom_in_return_button.clicked.connect(lambda: self.zoom_in_channel(0))
...
#%% RECORDING MODE FUNCTIONS
    ...
    def start_recording_mode_function(self):
        ...
        try:
            ...
            self.show_record_textedit.append(text)                                                              # Displays a message in the edit line of the interface
            return
        ...
        text = str('\nRECORDING MODE\nSignals are being collected and data is being saved.\n')                  # Message to be displeyed
        self.show_record_textedit.append(text)                                                                  # Displays a message in the edit line of the interface
    ...
    def plot_recording_mode_function(self):
        try:
            ...
            text = str('\nWARNING: The interface failed when trying to reproduce the graph')                    # Message to be displeyed
            self.show_record_textedit.append(text)                                                              # Displays a message in the edit line of the interface          
...
#%% PLOT CONTROLS FUNCTIONS
        
    def zoom_in_channel(self, number):
        if number == 0:
            self.plot_viewer.canvas.axes.set_ylim((0.1, 32.9))
            self.plot_viewer.canvas.axes.yaxis.set_minor_locator(AutoMinorLocator(1))
            self.plot_viewer.canvas.axes.grid(visible=False, which='minor')
            self.plot_viewer.canvas.axes.grid(visible=True,axis='y',which='major',alpha=0.2,ls='-',lw=0.6)
            self.plot_viewer.canvas.axes.set_ylabel("Channels", family = "DejaVu Sans", 
                                                    weight = "bold", color = "#FFFFFF", size = 6)                # Changes the labels design
        else:
            self.plot_viewer.canvas.axes.set_ylim((number - 0.15, number + 0.15))
            self.plot_viewer.canvas.axes.yaxis.set_minor_locator(AutoMinorLocator(1/(100*1e-4)))
            self.plot_viewer.canvas.axes.grid(visible=True,which='minor',axis='y',alpha=0.5,ls='--',lw=0.5)
            self.plot_viewer.canvas.axes.grid(visible=True,which='major',axis='y',alpha=0.8,ls='-',lw=1.5)
            self.plot_viewer.canvas.axes.set_ylabel("Voltage [grid scale 500uV]", family = "DejaVu Sans", 
                                                    weight = "bold", color = "#FFFFFF", size = 6)                # Changes the labels design 
...
#%% TIMER FUNCTIONS
    ...
    def start_timers_function(self):
        ...
        self.timer.setInterval(self.options.get_total_time()*1000 + 100)                                        # The timer is in milliseconds, so multiply the total time by 1000
        ...
        self.update_statistics_timer.setInterval(int(self.options.get_total_time()*100))
...
#%% CONTOL FUNCTIONS  
    ...
    def stop_function(self): 
        if self.stop_button.text() == "STOP":
            ...
            text = str('\nThe data acquisiiton was stopped\n')                                                  # Message to be displeyed
            self.show_record_textedit.append(text)                                                              # Displays a message in the edit line of the interface          
            self.stop_button.setText("START")
        else:
            self.data_worker.continue_record()                                                                  # 
            text = str('\nData acquisition is being done again\n')                                              # Message to be displeyed
            self.show_record_textedit.append(text)                                                              # Displays a message in the edit line of the interface          
