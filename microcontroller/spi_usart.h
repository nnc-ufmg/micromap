...
void USART0config(int CSel) {
  ...
  pinMode(CSel,OUTPUT);
  digital_write_direct(CSel,HIGH);
...
uint16_t USART0txfer(uint16_t txBytes,int CSel ) {
  ...
  digital_write_direct(CSel, LOW); // set CS pin LOW (begin SPI transfer)
  ...
  digital_write_direct(CSel, HIGH); // set CS pin HIGH (end SPI transfer)
...
*/
uint16_t transfer_dataUSART0(int CSel,uint16_t *XXbufferOUT,uint16_t *XXbufferIN,int xxCount) {
